...
def compute_citations_number(publis, sortedresult_file_path):
    citation_nb = 0
    not_evaluated = 0
    h_index = 0
    i_index = 0
    sorted_publis =  sorted(publis, key=lambda publi: publi['citation_count'], reverse=True)
    with open (sortedresult_file_path, 'w') as outfile:
        json.dump(sorted_publis, outfile)
...
def main():
    # Get the input
    date = '17Jan2016'
    index_file_path = './data/'+date+'/index-g5k.json'
    result_file_path = './results/'+date+'/result.json'
    sortedresult_file_path = './results/'+date+'/sorted_result.json'
    try:
    ...
    try:
        with open(index_file_path) as data_file:
   ...
   # publis_year = add_year(data, publis)
    ...
    compute_citations_number(publis_year, sortedresult_file_path)
