...
def make_scholar_request(title):
    ...
    if (querier.send_query(query)) == True :
        if(len(querier.articles) >= 1):
            citations_nb = -1
            # If the data I retrieved are correct (i.e. they correspond to the publication, I'm looking for, I update the citations_nb
            try:
                if (querier.articles[0]['title'].upper() == title.upper()):
                    citations_nb = querier.articles[0]['num_citations']
                else:
                   print 'Title does not match('+querier.articles[0]['title'].upper()+'/'+title.upper()+')'
                   error = True
            except:
                error = True 
            if error == True:
...
def check_nonevaluated(list_publis, result_file_path):
    i = 0  
    for publi in list_publis:
        if(publi['citation_count'] <= -1):
            publi['citation_count']= make_scholar_request(publi['title'])

            line = {'title': publi['title'], 'citation_count': publi['citation_count']}
            print line
            i += 1
...
def testmain():
    ...
    make_scholar_request('Efficient XML query processing') 
...
def main(): 
    ...
    try:
        ...
        with open('data/'+date+'/index.json') as data_file:
            data_non_filtered = json.load(data_file)
            data = filter_G5K_pub(data_non_filtered)
            with open ('data/'+date+'/index-g5k.json', 'w') as outfile_g5k:
                json.dump(data, outfile_g5k)

    # Get the input
    result_file_path = './results/'+date+'/result.json'
    ...
    print 'Check non evaluated'
    shutil.copy(result_file_path, result_file_path+'.bak')
    check_nonevaluated(result, result_file_path)
